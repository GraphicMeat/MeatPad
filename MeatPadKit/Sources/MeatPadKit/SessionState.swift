...
public struct SessionState: Codable, Equatable, Sendable {
    ...
    public init(openNoteIDs: [UUID], browserOpen: Bool, openProjects: [ProjectSession] = []) {
    ...
    private enum CodingKeys: String, CodingKey {
        case openNoteIDs, browserOpen, openProjects
    }

    // Custom decode: synthesized Codable doesn't fall back to a property's default value
    // for a missing key, so v1 session.json files (written before openProjects existed)
    // would otherwise fail to decode entirely. `encode(to:)` stays synthesized.
